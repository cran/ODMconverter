...
ODM2R <- function( ODMfile="" )
{
   ...
   StudyDescription <- xmlValue(ODM[["Study"]][["GlobalVariables"]][["StudyDescription"]])
	...
	# VALUE LABELS
      ...
      {
         contexts <- append(contexts, ainfo[[i]][2])
      }
      contexts <- unique(contexts)
      cat("attr(odmdata, \"Alias_Contexts\") <- c(\"")
      cat( paste(contexts, sep="",collapse="\",\""))
      cat("\")\n", sep="")	
      for (i in 1:length(contexts) )
      {
         cnames <- rep("",length(ivec))
         for (ii in 1:length(ainfo) )
         {
            if (contexts[i] == ainfo[[ii]][2])
            {
               for (iii in 1:length(ivec) )
               {
                  if (ivec[iii] == ainfo[[ii]][1])
                  {               
                     if (cnames[iii] == "") 
                     {
                        cnames[iii] <- ainfo[[ii]][3]
                     }
                     else
                     {
                        cnames[iii] <- paste(cnames[iii], ainfo[[ii]][3], sep=" ")
                     }
                  }
               }
            }
         }
         cat("attr(odmdata, \"", contexts[i], "\") <- c(\"", sep="")
         cat(cnames, sep="\",\"")
         cat("\")\n", sep="")	
      }
	sink()
